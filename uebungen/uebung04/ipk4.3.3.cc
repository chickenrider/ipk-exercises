...
void test_root(double q, int n, int steps)
{
	...
	std::cout<< "a="<< root_iterative(q,n,steps)<<std::endl;
	std::cout<< "~a=" << recursive(root_iterative(q,n,steps),n)<<std::endl;
	std::cout<< "q-~a="<< (q-(recursive(root_iterative(q,n,steps),n)))<<std::endl;
}
